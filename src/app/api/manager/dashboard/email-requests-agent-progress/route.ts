...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get progress data for each agent (EMAIL_REQUESTS tasks only)
    const agentProgress = await Promise.all(
      agents.map(async (agent) => {
        const [assigned, inProgress, completedToday, lastActivity] = await Promise.all([
          // Total assigned EMAIL_REQUESTS tasks (PENDING + IN_PROGRESS + ASSISTANCE_REQUIRED + COMPLETED)
          prisma.task.count({
            where: { 
          ...
          // Currently in progress EMAIL_REQUESTS tasks
          prisma.task.count({
            where: { 
              assignedToId: agent.id,
              status: 'IN_PROGRESS',
          ...
          // Completed today EMAIL_REQUESTS tasks
          prisma.task.count({
            where: {
              assignedToId: agent.id,
              status: 'COMPLETED',
          ...
          // Last activity (most recent EMAIL_REQUESTS task update)
    ...
    console.error('Error fetching Email Requests agent progress:', error);
    return NextResponse.json(
